...
# reportlab is optional; avoid crashing custom node import on HF Spaces.
try:
    from .nodes.captioning.FL_ImageCaptionLayoutPDF import FL_ImageCaptionLayoutPDF
except ModuleNotFoundError as _exc:
    # Keep ComfyUI startup resilient: this node is not needed for this Space.
    # Provide a tiny stub so NODE_CLASS_MAPPINGS remains valid.
    class FL_ImageCaptionLayoutPDF:  # type: ignore
        @classmethod
        def INPUT_TYPES(cls):
            return {"required": {}}
        ...
        RETURN_NAMES = ("error",)
        FUNCTION = "run"
        CATEGORY = "fill-nodes/disabled"

        def run(self):
            return (f"FL_ImageCaptionLayoutPDF disabled: missing dependency '{_exc.name}'",)
...
# ollama is optional; avoid crashing custom node import on HF Spaces.
try:
    from .nodes.captioning.FL_OllamaCaptioner import FL_OllamaCaptioner
except ModuleNotFoundError as _exc:
    class FL_OllamaCaptioner:  # type: ignore
        @classmethod
        def INPUT_TYPES(cls):
            return {"required": {}}
        ...
        def run(self):
            return (f"FL_OllamaCaptioner disabled: missing dependency '{_exc.name}'",)
