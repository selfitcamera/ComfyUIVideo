...
class ModelPatcher:
    ...
    def detach(self, unpatch_all=True):
        ...
        # During interpreter shutdown, imported module globals (CallbacksMP) can become None.
        # Avoid spurious "Exception ignored in: __del__" noise at exit.
        call_type = getattr(CallbacksMP, "ON_DETACH", None)
        if call_type is not None:
            for callback in self.get_all_callbacks(call_type):
    ...
    def __del__(self):
        # Cleanup is best-effort; during interpreter shutdown many globals may already be torn down.
        try:
            import sys
